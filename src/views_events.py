...
def working_with_transactions_events(request_time: str, period: str) -> dict[str, str | Any]:
    ...
    category_dictionary = (
        df[df["Сумма операции"] < 0].groupby("Категория")["Сумма операции"].sum().abs().sort_values(ascending=False)
    )
    main = [{"category": name, "amount": round(amount)} for name, amount in category_dictionary.head(7).items()]

    other_amount = round(category_dictionary.iloc[7:].sum()) if len(category_dictionary) > 7 else 0
    if other_amount > 0:
    ...
    category_dictionary_income = (
        df[df["Сумма операции"] > 0].groupby("Категория")["Сумма операции"].sum().sort_values(ascending=False)
    )
    ...
    json_str = {
        ...
        "income": {
            ...
            "main": [
                {"category": name, "amount": round(amount)} for name, amount in category_dictionary_income.items()
            ],
        },
