...
def receiving_cashback_data() -> list[dict]:
    ...
    try:
        df = pd.read_excel(data_path)[
            ["Дата платежа", "Кэшбэк", "Статус", "Категория", "Номер карты"]
        ]
...
def determination_of_cashback_categories(
    transactions: list[dict], year: str, month: str
) -> str:
    ...
    categories = set(
        i["Категория"] for i in transactions if fn(i["Дата платежа"]) == dt_search
    )
